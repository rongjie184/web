<?php
	class adminactionModel extends CI_Model{
    ...
    function get_showlist($parentid) {
        ...
        if($parentid){
            ...
            return $this->db->select()->where($arr)->get('admin_priv')->result_array();
        }else{
            $arr['pid'] = 0;
            return $this->db->select()->where($arr)->get('admin_priv')->result_array();
        }
    ...
    function edit_action($data,$action_id) {
        if($action_id){
            ...
            return $this->db->update('admin_priv', $data);
        }
        return array();
    }
    ...
    function action_exist($where){
        ...
        return $this->db->select()->where($where)->get('admin_priv')->row_array();

    }
    function add_action($data) {
        return $this->db->insert('admin_priv', $data);
    }
    /**
     * 获得所有顶级权限
     * @return [type] [description]
	...
	function parentlists() {
          ...
          return $this->db->select('id,name')->from('admin_priv')->where(array('pid'=>0))->get()->result_array();
    }
    ...
    public function get_list_page($start, $end=0,$where=array()) {
        ...
        return $this->db->select()->order_by('id','DESC')->from('admin_priv')->limit($end, $start)->get()->result_array();
    }
    /**
     * 获取权限列表总数
     * @param  [type] $where [description]
    ...
    public function action_count($where) {
        ...
        return $this->db->count_all_results('admin_priv');
