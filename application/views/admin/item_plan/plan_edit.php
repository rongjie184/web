...
<head>
  ...
  <title><?=$this->config->item('web_title')?> | 编辑项目进度</title>
            ...
            <form class="form-horizontal" method="post" action='<?=$this->form_action?>' id='iForm'>
              <div class="box-body">
                <div class="form-group">
                  <label for="name" class="col-sm-2 control-label">项目进度名称</label>
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="name" name='name' value='<?=$plan['name']?>' placeholder="请输入项目进度名称">
                  </div>
                </div>

				 <div class="form-group">
					  <label for="describe" class="col-sm-2 control-label">状态</label>
					  <div class="col-sm-5">
						<select id='status' name='status' class="form-control select2" style="width: 100%;">
						  <option value='1' <?php if($plan['status']==1){echo 'selected';}?>>正常</option>
						  <option value='0' <?php if($plan['status']==0){echo 'selected';}?>>关闭</option>
				 ...
				 <div class="form-group">
                  ...
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="order_num" name='order_num' value='<?=$plan['order_num']?>'placeholder="请输入序号">
                  </div>
                </div>
...
$(document).ready(function() {
   $('#iForm').formValidation({
     ...
     fields: {
        name:{
          validators: {
            notEmpty: {message: '项目进度不可为空'},
			 remote:{
              url: '/index.php/admin/item_plan/pnames_check',
              type: 'post',  
              delay:800,
			  data:{
