...
<head>
  ...
  <title><?=$this->config->item('web_title')?> | CRA项目中心管理</title>
        ...
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
			  ...
			  <input type="hidden" id="check" name="check" value="<?=$chcek?>"/>
				  <input type="hidden" id="uid" name="uid" />
              <table id="example1" class="table table-bordered table-striped">
                <thead>
                <tr>
                <td colspan=2><input type="text" name='cranames' value='<?=$cranames?>' class="form-control" placeholder="搜索 CRA姓名"></td>
                ...
                <?php
                  ...
                  <tr>
					...
					<td><?php if($check){?> <a class='btn btn-info btn-sm' href='javascript:void(0);' onclick="return out(<?=$value['id']?>)">退出</a> <?php }else{?>  
						<a class='btn btn-info btn-sm' href='javascript:void(0);' onclick="return out(<?=$value['id']?>)">退出</a>  <a class='btn btn-success btn-sm' href='javascript:void(0);' onclick="return enter(<?=$value['id']?>)">加入</a><?php }?></td>	
	 ...
	 function changeDatas(){  
	   $("#inits_id").find("option").remove();
        ...
        if( v == null){  
            ...
            // 省市联动部门下拉框  
            var url = "/index.php/admin/item_manage/get_iteminst?items_id=" + v;  
...
function enter(id){
	...
	if(confirm("确定要加入吗？")){
		document.dbform.action="/index.php/admin/item_manage/cro_lists_enter";
		dbform.submit();
	}
...
function out(id){
	...
	if(inits==0){alert('请选择中心');return false;}
	if(confirm("确定要退出吗？")){
		document.dbform.action="/index.php/admin/item_manage/cro_lists_out";
		dbform.submit();
	}
...
function check(){
	...
	if(items==0){alert('请选择项目');return false;}
	if(inits==0){alert('请选择中心');return false;}
	document.dbform.action="/index.php/admin/item_manage/cro_lists_check";
	dbform.submit();
