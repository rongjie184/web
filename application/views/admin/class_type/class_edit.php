...
<head>
  ...
  <title><?=$this->config->item('web_title')?> | 编辑药品种类</title>
            ...
            <form class="form-horizontal" method="post" action='<?=$this->form_action?>' id='iForm'>
              <div class="box-body">
                <div class="form-group">
                  <label for="name" class="col-sm-2 control-label">药品种类名称</label>
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="name" name='name' value='<?=$type['name']?>' placeholder="请输入药品种类名称">
                  </div>
                </div>
				...
				<div class="form-group">
                  ...
                  <div class="col-sm-4">
                   <select id='drug_id' name='drug_id' class="form-control select2">
						  <option value='0'>选择试验分类</option>
						  <?php
							foreach($drug as $key=>$value){	
								
						  ?>
						  <option value='<?=$key?>' <?php if($key==$type['drug_id']){echo 'selected';}?>><?=$value?></option>
						  <?php
...
$(document).ready(function() {
   $('#iForm').formValidation({
     ...
     fields: {
        name:{
          validators: {
            notEmpty: {message: '药品种类不可为空'},
			 remote:{
              url: '/index.php/admin/class_type/type_names_check',
              type: 'post',  
              delay:800,
			  data:{
