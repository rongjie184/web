...
<head>
  ...
  <title><?=$this->config->item('web_title')?> | 编辑试验类型</title>
            ...
            <form class="form-horizontal" method="post" action='<?=$this->form_action?>' id='iForm'>
              <div class="box-body">
                <div class="form-group">
                  <label for="name" class="col-sm-2 control-label">试验类型名称</label>
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="name" name='name' value='<?=$type['name']?>' placeholder="请输入试验类型名称">
                  </div>
                </div>
...
$(document).ready(function() {
   $('#iForm').formValidation({
     ...
     fields: {
        name:{
          validators: {
            notEmpty: {message: '试验类型不可为空'},
			 remote:{
              url: '/index.php/admin/test_type/type_names_check',
              type: 'post',  
              delay:800,
			  data:{
