...
<head>
  ...
  <title><?=$this->config->item('web_title')?> | 添加试验类型</title>
            ...
            <form class="form-horizontal" method="post" action='<?=$this->form_action?>' id='iForm'>
              <div class="box-body">
                <div class="form-group">
                  ...
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="name" name='name' placeholder="请输入试验类型名称">
                  </div>
                </div>
...
$(document).ready(function() {
   $('#iForm').formValidation({
     ...
     fields: {
        name:{
          validators: {
            notEmpty: {message: '试验类型名称不可为空'},
			remote:{
              url: '/index.php/admin/test_type/type_name_check',
              type: 'post',  
              delay:800,
