...
<head>
  ...
  <title><?=$this->config->item('web_title')?> | 编辑试验分类</title>
            ...
            <form class="form-horizontal" method="post" action='<?=$this->form_action?>' id='iForm'>
              <div class="box-body">
                <div class="form-group">
                  <label for="name" class="col-sm-2 control-label">试验分类名称</label>
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="name" name='name' value='<?=$type['name']?>' placeholder="请输入试验分类名称">
                  </div>
                </div>
				...
				<div class="form-group">
                  ...
                  <div class="col-sm-4">
                   <select id='test_id' name='test_id' class="form-control select2">
						  <option value='0'>选择试验类型</option>
						  <?php
							foreach($test as $key=>$value){	
								
						  ?>
						  <option value='<?=$key?>' <?php if($key==$type['test_id']){echo 'selected';}?>><?=$value?></option>
						  <?php
				 ...
				 <div class="form-group">
                  ...
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="desc" name='desc' value='<?=$type['desc']?>'placeholder="请输入描述">
                  </div>
                </div>
                ...
                <div class="form-group disabled">
                  ...
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="order_num" name='order_num' value='<?=$type['order_num']?>' placeholder="请输入显示排序">
                  </div>
                </div>
				-->
                <div class="form-group">
                  ...
                  <div class="col-sm-5">
                    ...
                    <select class="form-control" id="status" name='status'>
                        <option <?=$type['status']==1?'selected':''?> value=1>正常</option>
                        <option <?=$type['status']==0?'selected':''?> value=0>关闭</option>
                    </select>
                  </div>
                </div>
...
$(document).ready(function() {
   $('#iForm').formValidation({
     ...
     fields: {
        name:{
          validators: {
            notEmpty: {message: '试验分类不可为空'},
			 remote:{
              url: '/index.php/admin/drug_type/type_names_check',
              type: 'post',  
              delay:800,
			  data:{
