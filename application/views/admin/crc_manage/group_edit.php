...
<head>
  ...
  <title><?=$this->config->item('web_title')?> | 编辑CRC组别</title>
            ...
            <form class="form-horizontal" method="post" action='<?=$this->form_action?>' id='iForm'>
              <div class="box-body">
                <div class="form-group">
                  <label for="name" class="col-sm-2 control-label">组别名称</label>
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="gname" name='gname' value='<?=$group['gname']?>' placeholder="请输入组别名称">
                  </div>
                </div>
                    ...
                    <select id='area_id' name='area_id' class="form-control select2" style="width: 100%;">
						  ...
						  <?php
							foreach($area as $key=>$value){
								if($key==$group['area_id']){$opt='selected';}
				 ...
				 <div class="form-group">
                  ...
                  <div class="col-sm-5">
                    <input type="text" class="form-control" id="desc" name='desc' value='<?=$group['desc']?>'placeholder="请输入描述">
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-5">
                    ...
                    <select class="form-control" id="status" name='status'>
                        <option <?=$group['status']==1?'selected':''?> value=1>正常</option>
                        <option <?=$group['status']==0?'selected':''?> value=0>关闭</option>
                    </select>
                  </div>
                </div>
...
$(document).ready(function() {
   $('#iForm').formValidation({
     ...
     fields: {
        gname:{
          validators: {
            notEmpty: {message: '组别名称不可为空'},
			 remote:{
              url: '/index.php/admin/crc_manage/group_names_check',
              type: 'post',  
              delay:800,
			  data:{
                    id:function(){return $('#id').val()},
				    gname:function(){return $('#gname').val()}
				
               },
              message:'组别名称不合法或已存在,请更换'                    
            }
